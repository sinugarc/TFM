...
def sumgate1(n: int) -> QuantumCircuit:
    qubits=2*n+1
    qc = QuantumCircuit(qubits)
    for i in range(n):
        qc.cx(i,(n+1)+i)
...
def BV_TestMT1_Output(qc: QuantumCircuit, n: int):
    suma = sumgate(n)
...
def BV_MT1(mutant:QuantumCircuit, inputs: [str]) -> QuantumCircuit:
    ...
    inp=BV_TestMT_Input(xor(inputs[0],inputs[1]))
    qc.append(inp,q_test)
    
    qc.barrier()
    
    BV_TestMT1_Output(qc,n)
...
def BV_MT3(mutant:QuantumCircuit, inputs: [str]) -> QuantumCircuit:
    ...
    q_test=QuantumRegister(n, 'q_test')
    c_test=ClassicalRegister(n, 'c_test')
    qc = QuantumCircuit(qs, anc1, q_test, c_test)

    auxInp = QuantumCircuit(qs, anc1)
    f_s=BV_Oracle_Generator(inputs[0])
    auxInp.append(f_s, range(n+1))
    f_s1=BV_Oracle_Generator(inputs[1])
    auxInp.append(f_s1, range(n+1))
    gate_s = QCSetUp(mutant,auxInp)
    gate_s.name='  BV_s+s1  \n \ns='+ inputs[0]+'\n \ns1='+ inputs[1]
    qc.append(gate_s, range(n+1))
