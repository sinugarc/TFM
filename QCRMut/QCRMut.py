...
# Num_Elm will support us with the weights on random.choice operator
...
def mutant_generator(testQC: QuantumCircuit, num_mutants: int, seed: int = 1, save: bool = False, dir_name: str = "") -> Optional[Iterable[QuantumCircuit]]:
    """
    testQC : QuantumCircuit, QC under test
    ...
    if len(testQC.data) == 0 or len(MutableGateSetIndex) == 0:
        ...
        if save:
            for i in range(num_mutants):
                mutant = mutant_gen_insert_gate(testQC)
                saveMutant(mutant,f"{dir_name}/mutant_{str(i)}.pgz")
        else: 
...
def obtainMutant(path: str) -> QuantumCircuit:
